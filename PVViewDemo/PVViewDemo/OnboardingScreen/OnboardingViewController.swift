...
class OnboardingViewController: UIViewController {
    ...
    private func loadItemViews() {
        ...
        itemViews[ParallaxItem.popup1.identifier]?.center = CGPoint(x: iphoneView.bounds.width, y: 40)
        itemViews[ParallaxItem.popup2.identifier]?.center = CGPoint(x: 0, y: iphoneView.bounds.midY)
        
        /*
        [(ParallaxItem.popup1, CGPoint(x: iphoneView.bounds.width, y: 40)),
         (.popup2, CGPoint(x: 0, y: iphoneView.bounds.midY)),
         (.contactIcon, CGPoint(x: center.x - 10, y: center.y - 15)),
    ...
    func loadActions() {
        ...
        //Page 3
        let p3_iphone = [PVActionGroup(actions: [PVActionRotate(fromZ: Double.pi / 5), PVActionRotate(fromY: Double.pi / 6), PVActionScale(to: 0.3), PVActionMove(fromPosition: PVPoint(x: midX, y: midY), toPosition: PVPoint(x: midX - 70, y: midY + 50))], parameters: PVParameters(timingFunction: PVTimingFunction(name: .easeInOutBack)))]
...
extension OnboardingViewController: PVViewDelegate {
    ...
    func parallaxView(_ parallaxView: PVView, didEndTransitionFrom previousPageIndex: Int?) {
        ...
        if currentIndex == 0 && previousPageIndex == nil {
            ...
            if let popup1 = itemViews[ParallaxItem.popup1.identifier] {
            ...
            if let popup2 = itemViews[ParallaxItem.popup2.identifier] {
